#!/usr/bin/env python3
"""Export a Megatron torch_dist checkpoint iteration to Hugging Face safetensors.

This bundled runner is adapted from slime's public export entrypoint. It uses
the installed slime package and does not require the original checkout's
`tools/` directory.
"""
    ...
    def set_up_planner(self, state_dict, metadata=None, is_coordinator=False) -> None:
        for key, value in metadata.state_dict_metadata.items():
            if "optimizer" in key or "_state" in key:
                continue
            print(f"find {key} in torch_dist ckpt")
...
def save_tensors(args, model_name, state_dict, output_dir, chunk_size, vocab_size=None, origin_hf_dir=None):
    ...
    for name, param in get_named_params(args, state_dict):
        ...
        for converted_name, converted_param in convert_to_hf(args, model_name, name, param):
            ...
            if tensor_size + current_size > chunk_size:
                ...
                current_size = 0
            model_tensors[-1][converted_name] = converted_param
            current_size += tensor_size
            total_size += tensor_size

    if origin_hf_dir is not None:
        for filename in [f for f in os.listdir(origin_hf_dir) if f.endswith(".safetensors")]:
            with safetensors.safe_open(os.path.join(origin_hf_dir, filename), framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key not in converted_names:
                        print(f"add {key} from origin hf checkpoint")
                        converted_param = f.get_tensor(key)
                        converted_names.add(key)
                        tensor_size = converted_param.numel() * converted_param.element_size()
                        if tensor_size + current_size > chunk_size:
                            model_tensors.append({})
                            current_size = 0
                        model_tensors[-1][key] = converted_param
                        current_size += tensor_size
                    total_size += tensor_size
    ...
    for i, tensors in enumerate(model_tensors):
        ...
        print(f"{filename} saved in {time.time() - start:.2f} sec.")
...
def main() -> int:
    ...
    if args.model_name is None and args.origin_hf_dir is None:
        raise ValueError("Either --model-name or --origin-hf-dir must be provided.")
