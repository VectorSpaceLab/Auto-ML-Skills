...
| OpenAI-compatible server, chat/completions/responses/embeddings/models/health/metrics/tokenize/detokenize/audio | `vllm-openai-serving` | `references/workflows.md`, `references/endpoints.md` | `start_server.py`, `client_smoke.py` | Deep |
| Engine/server args, YAML config, tensor parallel, dtype, max model len, quantization, model aliases, tokenizer/chat template | `vllm-serving-config` | `references/workflows.md`, `references/engine-args.md` | `make_serve_config.py`, `validate_config.py` | Deep |
...
| Structured outputs/guided decoding/JSON/regex/grammar/tool calls/reasoning parsers/Responses payloads/backend choice | `vllm-structured-outputs` | `references/workflows.md`, `references/guided-decoding.md` | `make_structured_payload.py`, `validate_schema.py` | Deep |
| Embeddings/pooling/classification/reward/reranking/score/generative scoring | `vllm-embeddings-pooling` | `references/workflows.md`, `references/pooling-reference.md` | `make_embedding_payload.py`, `score_payload.py` | Deep |
| Multimodal image/video/audio/speech/realtime payloads, prompt embeddings, processor kwargs | `vllm-multimodal` | `references/workflows.md`, `references/multimodal-reference.md` | `make_mm_payload.py`, `validate_media_payload.py` | Medium |
| Distributed serving, Ray, Ray Serve, multiprocessing, TP/PP/DP/EP, multi-node, K8s, DBO, disaggregated prefill, NIXL/KV transfer | `vllm-distributed-serving` | `references/workflows.md`, `references/distributed-reference.md` | `make_distributed_command.py`, `check_cluster_env.py` | Medium |
| KV cache, prefix caching, chunked prefill, speculative decoding, AWQ/GPTQ/FP8/weight quant, quantized KV, torch.compile, CUDA graphs, DBO | `vllm-performance-tuning` | `references/workflows.md`, `references/performance-reference.md` | `make_perf_config.py`, `estimate_kv_cache.py` | Deep |
