#!/usr/bin/env python3
"""Check repository files against a maximum line limit.

This is a self-contained maintenance helper for Docling-like checkouts. Run it
from the checkout root, optionally passing explicit files. It mirrors the
repository guard's behavior without requiring access to the original source
script.
"""
...
def load_ignore_patterns(ignore_file: Path) -> tuple[list[str], list[str]]:
    """Load silent and warning-only glob patterns from an ignore file."""
...
def filter_candidate_files(repo_root: Path, file_args: list[str]) -> list[Path]:
    """Resolve file arguments to checked files under the checkout root."""
    files: list[Path] = []
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "files",
        nargs="*",
        help="Optional file paths to check. Defaults to scanning the checkout.",
    )
...
def main() -> int:
    ...
    if warnings:
        print(
            ...
            "(split or refactor before growing them):"
        )
    ...
    if violations:
        ...
        print(f"Add deliberate exceptions to {args.ignore_file} only for accepted debt.")
        return 1
