...
Read this when a FlashRAG workflow fails before reaching a sub-skill-specific error.

## Import Or Install Fails

- Confirm the active Python environment can import `flashrag`.
- Reinstall from a public package or public source checkout; do not rely on private path state.
- Run `python scripts/check_flash_rag_env.py` and fix missing required packages before training or serving.
