---
name: flashrag-reasoning-pipeline-skill
description: "Use when a user wants FlashRAG Reasoning-search pipelines such as SimpleDeepSearcher, SearchR1, AutoRefine, O2Searcher, ReaRAG, and CoRAG."
disable-model-invocation: true
---

# Reasoning-search pipelines such as SimpleDeepSearcher, SearchR1, AutoRefine, O2Searcher, ReaRAG, and CoRAG.

Use this sub-skill after the root `flash-rag` router selects `flashrag-reasoning-pipeline-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_reasoning_config.py](scripts/make_reasoning_config.py): bundled helper; run `python scripts/make_reasoning_config.py --help` before use.
- [scripts/run_fake_reasoning_pipeline.py](scripts/run_fake_reasoning_pipeline.py): bundled helper; run `python scripts/run_fake_reasoning_pipeline.py --help` before use.
- [scripts/validate_reasoning_data.py](scripts/validate_reasoning_data.py): bundled helper; run `python scripts/validate_reasoning_data.py --help` before use.
