...
Read this when executing `flashrag-multimodal-pipeline-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/make_mm_config.py`: run `python scripts/make_mm_config.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/run_fake_mm_pipeline.py`: run `python scripts/run_fake_mm_pipeline.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/validate_mm_dataset.py`: run `python scripts/validate_mm_dataset.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
python scripts/check_env.py
python ../../scripts/make_demo_data.py --data-dir <work_dir>/dataset --dataset-name tiny_qa --corpus <work_dir>/corpus.jsonl
python scripts/validate_mm_dataset.py --data <work_dir>/dataset/tiny_qa/test.jsonl
python scripts/make_mm_config.py --output <work_dir>/config.yaml --data-dir <work_dir>/dataset --dataset-name tiny_qa --save-dir <work_dir>/outputs
python scripts/run_fake_mm_pipeline.py --config <work_dir>/config.yaml --summary <work_dir>/summary.json
