...
Read this when executing `flashrag-reranker-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/inspect_reranked.py`: run `python scripts/inspect_reranked.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/make_rerank_config.py`: run `python scripts/make_rerank_config.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/offline_rerank.py`: run `python scripts/offline_rerank.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/validate_retrieval_input.py`: run `python scripts/validate_retrieval_input.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
python scripts/check_env.py
python ../../scripts/make_demo_data.py --data-dir <work_dir>/dataset --dataset-name tiny_qa --corpus <work_dir>/corpus.jsonl
python scripts/validate_retrieval_input.py --data <work_dir>/dataset/tiny_qa/test.jsonl
python scripts/make_rerank_config.py --output <work_dir>/config.yaml --data-dir <work_dir>/dataset --dataset-name tiny_qa --save-dir <work_dir>/outputs
python scripts/inspect_reranked.py --summary <work_dir>/summary.json
```
