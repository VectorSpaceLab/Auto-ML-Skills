...
Read this when executing `flashrag-multiturn-generator-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/make_multiturn_config.py`: run `python scripts/make_multiturn_config.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/make_multiturn_messages.py`: run `python scripts/make_multiturn_messages.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/run_fake_multiturn.py`: run `python scripts/run_fake_multiturn.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/validate_messages.py`: run `python scripts/validate_messages.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
mkdir -p <work_dir>
python scripts/check_env.py
python scripts/make_multiturn_config.py --output <work_dir>/multiturn.yaml --save-dir <work_dir>/outputs --framework hf --generator-model <model_name> --model-path <model_path> --max-new-tokens 64
python scripts/make_multiturn_messages.py --output <work_dir>/messages.json --turn1 "Who wrote Hamlet?" --turn2 "Give only the surname."
python scripts/validate_messages.py --messages <work_dir>/messages.json
python scripts/run_fake_multiturn.py --config <work_dir>/multiturn.yaml --messages <work_dir>/messages.json --summary <work_dir>/multiturn-summary.json
python scripts/inspect_output.py --summary <work_dir>/multiturn-summary.json
```

For single-turn generator prompts, return to the root router and choose `flashrag-generator-skill`. This sub-skill owns conversation-shaped message validation and multi-turn smoke execution.
