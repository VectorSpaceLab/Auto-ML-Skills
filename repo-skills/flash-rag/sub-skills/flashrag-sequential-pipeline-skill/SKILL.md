---
name: flashrag-sequential-pipeline-skill
description: "Use when a user wants FlashRAG Sequential retrieve-generate-evaluate RAG pipeline smoke and real-run transition."
disable-model-invocation: true
---

# Sequential retrieve-generate-evaluate RAG pipeline smoke and real-run transition.

Use this sub-skill after the root `flash-rag` router selects `flashrag-sequential-pipeline-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_pipeline_config.py](scripts/make_pipeline_config.py): bundled helper; run `python scripts/make_pipeline_config.py --help` before use.
- [scripts/run_fake_pipeline.py](scripts/run_fake_pipeline.py): bundled helper; run `python scripts/run_fake_pipeline.py --help` before use.
- [scripts/validate_pipeline_data.py](scripts/validate_pipeline_data.py): bundled helper; run `python scripts/validate_pipeline_data.py --help` before use.
