...
def main() -> int:
    ...
    if args.summary:
        ...
        ok = payload.get("records", 0) > 0 and payload.get("first_candidate_count", 0) >= 2
    ...
    if data_path.exists():
        ...
        if not rows:
            ...
            for key in ["retrieval_result", "candidates", "all_summary", "val_scores", "ranking_scores", "pred", "metric_score"]:
                present = key in output
                print(f"first_output_has_{key}: {str(present).lower()}")
                if not present:
                    errors.append(f"first record missing output.{key}")
            if len(output.get("candidates", [])) < 2:
                errors.append("first record has fewer than two candidates")
    if errors:
        print("valid: false")
        for error in errors:
            print(f"- {error}")
