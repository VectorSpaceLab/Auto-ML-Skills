---
name: flashrag-multi-retriever-skill
description: "Use when a user wants FlashRAG BM25+dense/CLIP multi-retriever configuration and merge settings."
disable-model-invocation: true
---

# BM25+dense/CLIP multi-retriever configuration and merge settings.

Use this sub-skill after the root `flash-rag` router selects `flashrag-multi-retriever-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/inspect_multi_config.py](scripts/inspect_multi_config.py): bundled helper; run `python scripts/inspect_multi_config.py --help` before use.
- [scripts/make_multi_retriever_config.py](scripts/make_multi_retriever_config.py): bundled helper; run `python scripts/make_multi_retriever_config.py --help` before use.
