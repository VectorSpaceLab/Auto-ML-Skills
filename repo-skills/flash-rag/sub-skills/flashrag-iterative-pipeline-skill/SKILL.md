---
name: flashrag-iterative-pipeline-skill
description: "Use when a user wants FlashRAG Iterative multi-round retrieval-generation pipeline smoke."
disable-model-invocation: true
---

# Iterative multi-round retrieval-generation pipeline smoke.

Use this sub-skill after the root `flash-rag` router selects `flashrag-iterative-pipeline-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_iterative_config.py](scripts/make_iterative_config.py): bundled helper; run `python scripts/make_iterative_config.py --help` before use.
- [scripts/offline_import_stubs.py](scripts/offline_import_stubs.py): support module imported by nearby scripts; read or adapt it only when modifying helper behavior.
- [scripts/run_fake_iterative_pipeline.py](scripts/run_fake_iterative_pipeline.py): bundled helper; run `python scripts/run_fake_iterative_pipeline.py --help` before use.
- [scripts/validate_iterative_data.py](scripts/validate_iterative_data.py): bundled helper; run `python scripts/validate_iterative_data.py --help` before use.
