...
Read this when executing `flashrag-iterative-pipeline-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/make_iterative_config.py`: run `python scripts/make_iterative_config.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/offline_import_stubs.py`: support module imported by nearby scripts; read or adapt it only when modifying helper behavior.
- `scripts/run_fake_iterative_pipeline.py`: run `python scripts/run_fake_iterative_pipeline.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/validate_iterative_data.py`: run `python scripts/validate_iterative_data.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
python scripts/check_env.py
python ../../scripts/make_demo_data.py --data-dir <work_dir>/dataset --dataset-name tiny_qa --corpus <work_dir>/corpus.jsonl
python scripts/validate_iterative_data.py --data <work_dir>/dataset/tiny_qa/test.jsonl
python scripts/make_iterative_config.py --output <work_dir>/iter.yaml --data-dir <work_dir>/dataset --dataset-name tiny_qa --save-dir <work_dir>/outputs --sample-num 2
python scripts/run_fake_iterative_pipeline.py --config <work_dir>/iter.yaml --summary <work_dir>/iter_summary.json
python scripts/inspect_output.py --summary <work_dir>/iter_summary.json
```
