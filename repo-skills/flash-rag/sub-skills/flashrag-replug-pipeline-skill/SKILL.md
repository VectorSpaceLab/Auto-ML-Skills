---
name: flashrag-replug-pipeline-skill
description: "Use when a user wants FlashRAG REPLUG document-score weighted generation workflows."
disable-model-invocation: true
---

# REPLUG document-score weighted generation workflows.

Use this sub-skill after the root `flash-rag` router selects `flashrag-replug-pipeline-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_replug_config.py](scripts/make_replug_config.py): bundled helper; run `python scripts/make_replug_config.py --help` before use.
- [scripts/run_fake_replug_pipeline.py](scripts/run_fake_replug_pipeline.py): bundled helper; run `python scripts/run_fake_replug_pipeline.py --help` before use.
- [scripts/validate_replug_data.py](scripts/validate_replug_data.py): bundled helper; run `python scripts/validate_replug_data.py --help` before use.
