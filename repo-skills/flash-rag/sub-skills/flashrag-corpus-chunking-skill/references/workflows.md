...
Read this when executing `flashrag-corpus-chunking-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/chunk_corpus.py`: run `python scripts/chunk_corpus.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/inspect_chunks.py`: run `python scripts/inspect_chunks.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/validate_doc_corpus.py`: run `python scripts/validate_doc_corpus.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
python scripts/check_env.py
python ../../scripts/make_demo_data.py --data-dir <work_dir>/dataset --dataset-name tiny_qa --corpus <work_dir>/corpus.jsonl
python scripts/validate_doc_corpus.py --input <work_dir>/corpus.jsonl
python scripts/chunk_corpus.py --input <work_dir>/corpus.jsonl --output <work_dir>/chunked.jsonl --chunk-by word --chunk-size 80
python scripts/inspect_chunks.py --output <work_dir>/chunked.jsonl
```
