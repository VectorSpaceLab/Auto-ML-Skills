...
Read this when executing `flashrag-dense-retrieval-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/inspect_dense_output.py`: run `python scripts/inspect_dense_output.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/make_dense_config.py`: run `python scripts/make_dense_config.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/make_dense_index_command.py`: run `python scripts/make_dense_index_command.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/run_dense_search.py`: run `python scripts/run_dense_search.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
python scripts/make_dense_index_command.py --corpus <work_dir>/corpus.jsonl --retrieval-method <method> --model-path <model_path> --save-dir <index_dir> --output <work_dir>/build_dense_index.sh
bash <work_dir>/build_dense_index.sh
python scripts/make_dense_config.py --output <work_dir>/dense.yaml --corpus <work_dir>/corpus.jsonl --index-path <index_dir>/<index_file> --model-path <model_path> --save-dir <work_dir>/outputs
