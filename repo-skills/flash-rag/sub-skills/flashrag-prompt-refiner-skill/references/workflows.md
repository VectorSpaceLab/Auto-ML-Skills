...
Read this when executing `flashrag-prompt-refiner-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/doc_utils.py`: support module imported by nearby scripts; read or adapt it only when modifying helper behavior.
- `scripts/inspect_prompt.py`: run `python scripts/inspect_prompt.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/make_prompt_config.py`: run `python scripts/make_prompt_config.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
- `scripts/run_offline_refiner.py`: run `python scripts/run_offline_refiner.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/validate_retrieval_docs.py`: run `python scripts/validate_retrieval_docs.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
python scripts/check_env.py
python ../../scripts/make_demo_data.py --data-dir <work_dir>/dataset --dataset-name tiny_qa --corpus <work_dir>/corpus.jsonl
python scripts/validate_retrieval_docs.py --docs <work_dir>/corpus.jsonl
python scripts/make_prompt_config.py --output <work_dir>/prompt.yaml --save-dir <work_dir>/outputs
python scripts/render_prompt.py --config <work_dir>/prompt.yaml --question "What is RAG?" --docs <work_dir>/corpus.jsonl --output <work_dir>/prompt.json
python scripts/run_offline_refiner.py --config <work_dir>/prompt.yaml --question "What is RAG?" --docs <work_dir>/corpus.jsonl --output <work_dir>/refined_prompt.json
python scripts/inspect_prompt.py --output <work_dir>/refined_prompt.json
```
