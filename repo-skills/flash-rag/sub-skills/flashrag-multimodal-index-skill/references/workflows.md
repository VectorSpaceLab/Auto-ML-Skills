...
Read this when executing `flashrag-multimodal-index-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/inspect_index.py`: run `python scripts/inspect_index.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/make_mm_index_command.py`: run `python scripts/make_mm_index_command.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/preflight_mm_index.py`: run `python scripts/preflight_mm_index.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/run_fake_mm_index.py`: run `python scripts/run_fake_mm_index.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/validate_mm_corpus.py`: run `python scripts/validate_mm_corpus.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
python scripts/check_env.py
python ../../scripts/make_demo_data.py --data-dir <work_dir>/dataset --dataset-name tiny_qa --corpus <work_dir>/corpus.jsonl
python scripts/validate_mm_corpus.py --data <work_dir>/dataset/tiny_qa/test.jsonl
python scripts/run_fake_mm_index.py --config <work_dir>/config.yaml --summary <work_dir>/summary.json
python scripts/inspect_index.py --summary <work_dir>/summary.json
```
