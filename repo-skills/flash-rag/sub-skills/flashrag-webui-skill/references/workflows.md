...
Read this when executing `flashrag-webui-skill`. This reference is self-contained and assumes FlashRAG is installed from a package index or public repository, not from the original extraction checkout.
...
- `scripts/check_webui.py`: run `python scripts/check_webui.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/run_webui.py`: run `python scripts/run_webui.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
- `scripts/stop_webui.py`: run `python scripts/stop_webui.py --help`; use it for validation, config generation, fake smoke execution, real package API execution, inspection, or handoff as its name indicates.
...
mkdir -p <work_dir>
python scripts/check_env.py
python scripts/run_webui.py --work-dir <work_dir> --host 127.0.0.1 --port <port> --pid-file <work_dir>/webui.pid --log <work_dir>/webui.log
python scripts/check_webui.py --url http://127.0.0.1:<port>
python scripts/stop_webui.py --pid-file <work_dir>/webui.pid
```
