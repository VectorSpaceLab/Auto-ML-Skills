---
name: llamafactory-ppo-skill
description: "Use when a user wants LLaMA-Factory PPO / RLHF policy optimization that requires a reward model or reward API."
disable-model-invocation: true
---

# PPO / RLHF policy optimization that requires a reward model or reward API.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-ppo-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_ppo_config.py](scripts/make_ppo_config.py): bundled helper; run `python scripts/make_ppo_config.py --help` before use.
- [scripts/preflight_ppo_config.py](scripts/preflight_ppo_config.py): bundled helper; run `python scripts/preflight_ppo_config.py --help` before use.
- [scripts/run_ppo.py](scripts/run_ppo.py): bundled helper; run `python scripts/run_ppo.py --help` before use.
- [scripts/validate_ppo_data.py](scripts/validate_ppo_data.py): bundled helper; run `python scripts/validate_ppo_data.py --help` before use.
