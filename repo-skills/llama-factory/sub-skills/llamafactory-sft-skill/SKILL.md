---
name: llamafactory-sft-skill
description: "Use when a user wants LLaMA-Factory SFT / supervised fine-tuning / instruction tuning / chat fine-tuning."
disable-model-invocation: true
---

# SFT / supervised fine-tuning / instruction tuning / chat fine-tuning.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-sft-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/check_v1_env.py](scripts/check_v1_env.py): bundled helper; run `python scripts/check_v1_env.py --help` before use.
- [scripts/inspect_output.py](scripts/inspect_output.py): bundled helper; run `python scripts/inspect_output.py --help` before use.
- [scripts/lf_common.py](scripts/lf_common.py): support module imported by nearby scripts; read or adapt it only when modifying helper behavior.
- [scripts/make_v1_dataset.py](scripts/make_v1_dataset.py): bundled helper; run `python scripts/make_v1_dataset.py --help` before use.
- [scripts/make_v1_sft_config.py](scripts/make_v1_sft_config.py): bundled helper; run `python scripts/make_v1_sft_config.py --help` before use.
- [scripts/run_v1_sft.py](scripts/run_v1_sft.py): bundled helper; run `python scripts/run_v1_sft.py --help` before use.
- [scripts/validate_sft_data.py](scripts/validate_sft_data.py): bundled helper; run `python scripts/validate_sft_data.py --help` before use.
