...
from lf_common import build_env, run_streaming


def main() -> int:
    parser = argparse.ArgumentParser(description="Run LLaMA-Factory v1 SFT.")
    ...
    env = build_env(None, use_v1=True, extra_pythonpath=args.extra_pythonpath)
    if args.master_addr:
        env["MASTER_ADDR"] = args.master_addr
    if args.master_port:
        env["MASTER_PORT"] = args.master_port
    return run_streaming([args.python, "-m", "llamafactory.cli", "sft", str(args.config)], args.work_dir, env, args.log)
