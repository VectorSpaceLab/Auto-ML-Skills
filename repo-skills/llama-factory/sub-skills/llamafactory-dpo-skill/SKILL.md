---
name: llamafactory-dpo-skill
description: "Use when a user wants LLaMA-Factory DPO / ORPO / SimPO / pairwise preference optimization."
disable-model-invocation: true
---

# DPO / ORPO / SimPO / pairwise preference optimization.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-dpo-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_v0_dpo_config.py](scripts/make_v0_dpo_config.py): bundled helper; run `python scripts/make_v0_dpo_config.py --help` before use.
- [scripts/run_v0_train.py](scripts/run_v0_train.py): bundled helper; run `python scripts/run_v0_train.py --help` before use.
- [scripts/validate_dpo_data.py](scripts/validate_dpo_data.py): bundled helper; run `python scripts/validate_dpo_data.py --help` before use.
