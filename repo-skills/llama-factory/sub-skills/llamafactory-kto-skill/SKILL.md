---
name: llamafactory-kto-skill
description: "Use when a user wants LLaMA-Factory KTO training from binary desirable/undesirable feedback."
disable-model-invocation: true
---

# KTO training from binary desirable/undesirable feedback.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-kto-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_tiny_kto_dataset.py](scripts/make_tiny_kto_dataset.py): bundled helper; run `python scripts/make_tiny_kto_dataset.py --help` before use.
- [scripts/make_v0_kto_config.py](scripts/make_v0_kto_config.py): bundled helper; run `python scripts/make_v0_kto_config.py --help` before use.
- [scripts/run_v0_train.py](scripts/run_v0_train.py): bundled helper; run `python scripts/run_v0_train.py --help` before use.
- [scripts/validate_kto_data.py](scripts/validate_kto_data.py): bundled helper; run `python scripts/validate_kto_data.py --help` before use.
