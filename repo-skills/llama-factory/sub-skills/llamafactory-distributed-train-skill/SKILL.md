---
name: llamafactory-distributed-train-skill
description: "Use when a user wants LLaMA-Factory torchrun, DeepSpeed, FSDP/FSDP2, Ray, and multi-node launch setup."
disable-model-invocation: true
---

# torchrun, DeepSpeed, FSDP/FSDP2, Ray, and multi-node launch setup.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-distributed-train-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_distributed_config.py](scripts/make_distributed_config.py): bundled helper; run `python scripts/make_distributed_config.py --help` before use.
- [scripts/run_distributed.py](scripts/run_distributed.py): bundled helper; run `python scripts/run_distributed.py --help` before use.
