...
def stream(cmd: list[str], cwd: Path, env: dict[str, str], log: Path | None) -> int:
...
def inspect_output(path: Path) -> int:
    ...
    for name in sorted(names):
        print(f"- {name}")
    ok = bool(names.intersection({"config.json", "model.safetensors", "adapter_config.json"})) or any(
...
def check_env(package_root: Path | None = None, extra_pythonpath: list[str] | None = None) -> int:
    ...
    errors: list[str] = []
    for pkg in ["llamafactory", "torch", "transformers", "datasets", "accelerate", "peft", "trl"]:
        status = import_status(pkg)
        print(f"{pkg}: {version(pkg)}; {status}")
    if import_status("trl").startswith("import failed"):
        errors.append("default model loader imports trl; install it even for PT")
    datasets_ver = version("datasets")
    if datasets_ver != "not installed" and not ((2, 16, 0) <= version_tuple(datasets_ver) <= (4, 0, 0)):
        print("warnings:")
        print("- default trainer version check requires datasets>=2.16.0,<=4.0.0; fix the version or use --disable-version-check for smoke tests")
    if errors:
        print("valid: false")
        for error in errors:
            print(f"- {error}")
