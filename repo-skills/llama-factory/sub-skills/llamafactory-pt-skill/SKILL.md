---
name: llamafactory-pt-skill
description: "Use when a user wants LLaMA-Factory Continued pretraining / causal LM pretraining on raw text."
disable-model-invocation: true
---

# Continued pretraining / causal LM pretraining on raw text.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-pt-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_v0_pt_config.py](scripts/make_v0_pt_config.py): bundled helper; run `python scripts/make_v0_pt_config.py --help` before use.
- [scripts/run_v0_train.py](scripts/run_v0_train.py): bundled helper; run `python scripts/run_v0_train.py --help` before use.
- [scripts/validate_pt_data.py](scripts/validate_pt_data.py): bundled helper; run `python scripts/validate_pt_data.py --help` before use.
