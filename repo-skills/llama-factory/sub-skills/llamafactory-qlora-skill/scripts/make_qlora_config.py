#!/usr/bin/env python3
from __future__ import annotations

import argparse
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "scripts"))
from lf_skill_common import write_yaml


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate a LLaMA-Factory default-trainer QLoRA SFT config.")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--model", required=True)
    parser.add_argument("--dataset", default="identity,alpaca_en_demo")
    parser.add_argument("--dataset-dir", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--template", default="qwen3_nothink")
    parser.add_argument("--quantization-bit", type=int, choices=[2, 3, 4, 8], default=4)
    parser.add_argument("--quantization-method", choices=["bnb", "hqq", "eetq"], default="bnb")
    parser.add_argument("--double-quantization", action="store_true")
    parser.add_argument("--cutoff-len", type=int, default=2048)
    parser.add_argument("--max-samples", type=int, default=1000)
    parser.add_argument("--max-steps", type=int, default=None)
    parser.add_argument("--learning-rate", type=float, default=1e-4)
    parser.add_argument("--lora-rank", type=int, default=8)
    parser.add_argument("--lora-target", default="all")
    args = parser.parse_args()
    cfg = {
        "model_name_or_path": args.model,
        "quantization_bit": args.quantization_bit,
        "quantization_method": args.quantization_method,
        "double_quantization": args.double_quantization,
        "trust_remote_code": True,
        "flash_attn": "sdpa",
        "stage": "sft",
        "do_train": True,
        "finetuning_type": "lora",
        "lora_rank": args.lora_rank,
        "lora_target": args.lora_target,
        "dataset": args.dataset,
        "dataset_dir": args.dataset_dir,
        "template": args.template,
        "cutoff_len": args.cutoff_len,
        "max_samples": args.max_samples,
        "preprocessing_num_workers": 1,
        "dataloader_num_workers": 0,
        "output_dir": args.output_dir,
        "logging_steps": 1,
        "save_steps": 500,
        "plot_loss": False,
        "overwrite_output_dir": True,
        "save_only_model": False,
        "report_to": "none",
        "per_device_train_batch_size": 1,
        "gradient_accumulation_steps": 8,
        "learning_rate": args.learning_rate,
        "num_train_epochs": 3.0,
        "lr_scheduler_type": "cosine",
        "warmup_ratio": 0.1,
        "bf16": True,
        "ddp_timeout": 180000000,
    }
    if args.max_steps is not None:
        cfg["max_steps"] = args.max_steps
    write_yaml(args.output, cfg)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
