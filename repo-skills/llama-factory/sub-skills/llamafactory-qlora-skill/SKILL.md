---
name: llamafactory-qlora-skill
description: "Use when a user wants LLaMA-Factory QLoRA and 4-bit or 8-bit LoRA training."
disable-model-invocation: true
---

# QLoRA and 4-bit or 8-bit LoRA training.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-qlora-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_qlora_config.py](scripts/make_qlora_config.py): bundled helper; run `python scripts/make_qlora_config.py --help` before use.
- [scripts/run_train.py](scripts/run_train.py): bundled helper; run `python scripts/run_train.py --help` before use.
