---
name: llamafactory-adapter-variants-skill
description: "Use when a user wants LLaMA-Factory LoRA variants such as LoRA+, rsLoRA, DoRA, PiSSA, OFT, and adapter continuation."
disable-model-invocation: true
---

# LoRA variants such as LoRA+, rsLoRA, DoRA, PiSSA, OFT, and adapter continuation.

Use this sub-skill after the root `llama-factory` router selects `llamafactory-adapter-variants-skill`. It is focused on one workflow family and should be enough to validate inputs, generate/adapt configs, run a smoke test, inspect outputs, and then scale to the user's requested run without reopening the original repository docs.
...
- [scripts/make_adapter_config.py](scripts/make_adapter_config.py): bundled helper; run `python scripts/make_adapter_config.py --help` before use.
- [scripts/preflight_adapter_config.py](scripts/preflight_adapter_config.py): bundled helper; run `python scripts/preflight_adapter_config.py --help` before use.
- [scripts/run_adapter_train.py](scripts/run_adapter_train.py): bundled helper; run `python scripts/run_adapter_train.py --help` before use.
