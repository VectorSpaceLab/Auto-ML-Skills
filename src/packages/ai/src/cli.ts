...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @auto-ml-skills/disco-ai <command> [provider]
...
Examples:
  npx @auto-ml-skills/disco-ai login              # interactive provider selection
  npx @auto-ml-skills/disco-ai login anthropic    # login to specific provider
  npx @auto-ml-skills/disco-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @auto-ml-skills/disco-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @auto-ml-skills/disco-ai --help' for usage`);
	process.exit(1);
}
