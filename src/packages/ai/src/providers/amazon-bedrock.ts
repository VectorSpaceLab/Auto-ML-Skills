...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "disco-ai-custom-headers", priority: "low" });
}
 ...
 * Defaults to "short" and uses DISCO_CACHE_RETENTION when set. */
function resolveCacheRetention(cacheRetention?: CacheRetention): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (typeof process !== "undefined" && process.env.DISCO_CACHE_RETENTION === "long") {
		return "long";
	}
	return "short";
}
