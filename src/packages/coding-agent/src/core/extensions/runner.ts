...
import type { ImageContent, Model } from "@auto-ml-skills/disco-ai";
...
export class ExtensionRunner {
	...
	private executeToolFn: ExecuteToolHandler = async (name) => {
		throw new Error(`Tool execution is not available: ${name}`);
	};
	...
	createContext(): ExtensionContext {
		...
		return {
			...
			executeTool: (name, params, options) => {
				runner.assertActive();
				return runner.executeToolFn(name, params, options);
			},
		};
	}
