...
import type {
	...
	AgentTool,
	ThinkingLevel,
} from "@auto-ml-skills/disco-agent-core";
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@auto-ml-skills/disco-ai";
import {
	clampThinkingLevel,
...
export class AgentSession {
	 ...
	 * - Handles extension commands (registered via disco.registerCommand) immediately, even during streaming
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			// Extension commands manage their own LLM interaction via disco.sendMessage()
		...
		runner.bindCore(
			...
			},
			{
				...
				executeTool: async (name, params, options) => {
					...
					return tool.execute(
						options?.toolCallId ?? `extension:${name}`,
						params as never,
						options?.signal ?? this.agent.signal,
						options?.onUpdate,
					);
				},
