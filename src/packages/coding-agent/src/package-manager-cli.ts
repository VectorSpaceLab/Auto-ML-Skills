import { Markdown, type MarkdownTheme } from "@auto-ml-skills/disco-tui";
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self] [--self] [--extensions] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update DisCo and installed packages.

Options:
  --self                  Update DisCo only
...
Short forms:
  ${APP_NAME} update                Update DisCo and all extensions
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestDisCoRelease(VERSION);
