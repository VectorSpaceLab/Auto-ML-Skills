 ...
 *
 * The state machine and line rendering are pure and unit-tested; the disco-tui
...
import type { Component, Focusable, TUI } from "@auto-ml-skills/disco-tui";
import { parseKey, truncateToWidth, wrapTextWithAnsi } from "@auto-ml-skills/disco-tui";
...
export class NavigatorModel {
  ...
  constructor(
    ...
    storage?: { list(): SavedWorkflow[]; delete(name: string, location?: string): boolean },
  ) {
...
export function renderNavigator(
  ...
  if (state.kind === "runs") {
    ...
    if (w) {
      ...
      body.push(dim("Location: ") + (w.location === "user" ? "user (~/.disco)" : "project (~/.disco/workflows/projects)"));
...
import type { OverlayAnchor } from "@auto-ml-skills/disco-tui";
...
export function openWorkflowNavigator(
  ...
  return ui.custom<void>(
    (tui: TUI, theme: Theme, _keybindings, done: (r: undefined) => void) => {
      ...
      // screen-boundary contrast. Follows the same pattern as ask-user:
      //   top border ──╭───╮
      //   side borders │ … │
      //   bottom border╰───╯
      let _focused = false;
