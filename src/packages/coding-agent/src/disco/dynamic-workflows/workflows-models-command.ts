 ...
 *
 * Uses DisCo's built-in `ctx.ui.select()`, `ctx.ui.confirm()`, and `ctx.ui.notify()`
 ...
 * DisCo's `/model` command, so users see exactly the same models.
 *
 * Each tier holds exactly one model spec string.
 * When editing a tier, a single-select picker is used (like DisCo's `/model`).
 */
...
/**
 * Register the `/workflows-models` command with DisCo.
 */
export function registerWorkflowModelsCommand(pi: ExtensionAPI): void {
  pi.registerCommand("workflows-models", {
    ...
    handler: async (_args, ctx) => {
      ...
      // user's current DisCo model). Nothing is written to disk until the user
      ...
      while (true) {
        ...
        if (choice === "Reset to defaults") {
          const confirmed = await ctx.ui.confirm(
            "Reset model tiers",
            "This will reset every tier to your current DisCo model. Continue?",
          );
          if (confirmed) {
 ...
 *
 * Uses `ctx.ui.custom()` with DisCo TUI's `SelectList` for proper
