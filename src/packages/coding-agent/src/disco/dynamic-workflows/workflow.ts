...
export interface WorkflowRunOptions extends WorkflowAgentOptions {
   ...
   * run for determinism. Defaults to scanning `.disco/agents` (project and user).
...
export interface AgentOptions<TSchemaDef extends TSchema | undefined = TSchema | undefined> {
  label?: string;
  /** DisCo create-skill progress hint: the planned sub-skill this agent is writing or reviewing. */
  subSkill?: string;
  phase?: string;
  schema?: TSchemaDef;
  /**
  ...
  isolation?: "worktree";
  /**
   * Name of a registered subagent definition (`.disco/agents/<name>.md`, project >
...
function defaultAgentLabel(phase: string | undefined, index: number, subSkill?: string): string {
  if (subSkill) return `${subSkill} agent ${index}`;
...
function buildAgentInstructions(
  ...
  if (options.subSkill) lines.push(`DisCo sub-skill: ${options.subSkill}`);
