...
export function renderWorkflowLines(
  ...
  for (const phase of phaseNames) {
    ...
    for (const agent of visibleAgents) {
      ...
      lines.push(
        `    ${order} ${statusIcon(agent.status)}${subSkill} ${shorten(agent.label, 56)}${agentModel}${agentTokens}${agentError}${result}`,
      );
    }
  ...
  if (unphased.length) {
    ...
    for (const agent of unphased.slice(-maxAgents)) {
      ...
      const agentError = agent.error ? theme.fg("warning", ` — ${shorten(agent.error, 72)}`) : "";
      lines.push(
        `    [${agent.id}] ${statusIcon(agent.status)}${subSkill} ${shorten(agent.label, 56)}${agentModel}${agentTokens}${agentError}${result}`,
      );
    }
  }
