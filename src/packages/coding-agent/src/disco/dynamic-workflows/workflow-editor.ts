 ...
 *
 *  - When the explicit keyword trigger is enabled, editor text containing the
 *    word `workflow`/`workflows` renders those letters as a flowing rainbow,
 *    signalling that submitting will engage a workflow.
 ...
 *    instruct DisCo to actually run the workflow tool.
 *
 * Implementation: we replace the core editor with a thin subclass of the exported
 * `CustomEditor` (which itself extends disco-tui's `Editor`), overriding only
