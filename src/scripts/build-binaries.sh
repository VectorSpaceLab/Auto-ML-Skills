#!/usr/bin/env bash
#
# Build DisCo binaries for all platforms locally.
...
#     disco-darwin-arm64.tar.gz
#     disco-darwin-x64.tar.gz
#     disco-linux-x64.tar.gz
#     disco-linux-arm64.tar.gz
#     disco-windows-x64.zip
#     disco-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        "${BUN_CMD[@]}" build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/disco.exe"
    else
        "${BUN_CMD[@]}" build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/disco"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    ...
    cp -r dist/disco-resources/skills "$OUTPUT_DIR/$platform/disco-skills"
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating disco-$platform.zip..."
        (cd "$platform" && zip -r ../disco-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating disco-$platform.tar.gz..."
        mv "$platform" disco && tar -czf disco-$platform.tar.gz disco && mv disco "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../disco-$platform.zip)
    else
        tar -xzf disco-$platform.tar.gz && mv disco "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/disco.exe"
    else
        echo "  $OUTPUT_DIR/$platform/disco"
    fi
done
