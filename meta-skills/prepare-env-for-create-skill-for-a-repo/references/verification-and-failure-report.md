...
Read this reference after installation commands run. The central rule is simple: an environment is not ready for `create-skill-for-a-repo` until verification proves it can be used for live package inspection.

## Mandatory Verification Gates

A successful handoff requires all applicable gates to pass:

1. Conda prefix exists and is not `base`.
...
- `handoff.python_executable`: pass this to `create-skill-for-a-repo` as the temporary inspection Python.
...
```text
Environment ready for create-skill-for-a-repo: yes
...
```text
Environment ready for create-skill-for-a-repo: no
...
2. <fallback route if one exists>
...
- Any warnings that weaken later `create-skill-for-a-repo` inspection must be surfaced in the handoff.
