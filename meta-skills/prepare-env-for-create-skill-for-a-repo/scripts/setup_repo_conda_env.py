#!/usr/bin/env python3
"""
Create or reuse a conda prefix, install a local repo package, and verify it.
...
This helper is intentionally conservative:
  ...
  * It writes a private JSON report with commands, facts, warnings, failures,
    and a handoff block for create-skill-for-a-repo.
...
def host_probe(conda_exe: str) -> dict[str, Any]:
    probe: dict[str, Any] = {
        ...
        "conda_executable": shutil.which(conda_exe) or conda_exe,
    }

    for name, cmd in {
        "conda_version": [conda_exe, "--version"],
        "conda_info_json": [conda_exe, "info", "--json"],
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create/reuse a conda prefix, install a local repo package, and verify it.",
    )
    parser.add_argument("--repo", required=True, help="Local repository path.")
    parser.add_argument("--conda-prefix", required=True, help="Target conda environment prefix/path.")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        conda_path = shutil.which(args.conda) or args.conda
        if shutil.which(args.conda) is None and not Path(args.conda).exists():
            fail(report, "conda", f"Conda executable not found: {args.conda}")
            return finish(report, report_path)

        report["host"] = host_probe(conda_path)
        root_prefix = report["host"].get("conda_root_prefix")
        if root_prefix and Path(str(root_prefix)).expanduser().resolve() == prefix:
        ...
        if not prefix.exists() and not args.verify_only:
            ...
            if not ok(result):
                fail(report, "conda-create", "Failed to create conda prefix.", result)
                return finish(report, report_path)
        elif not prefix.exists() and args.verify_only:
            fail(report, "verify-only", f"Conda prefix does not exist: {prefix}")
            return finish(report, report_path)
        else:
            warn(report, "Conda prefix already exists; reusing it without deleting or recreating.")
